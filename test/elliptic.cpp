...
int main (int,char**)
{
	...
	f.setup (order, samplingrate, cutoff_frequency, 1,1);
	f.reset ();
	double b;
	...
	Iir::Elliptic::BandStop<order,Iir::DirectFormI> bs;
	const float center_frequency = 50;
	const float frequency_width = 5;
	bs.setup (order, samplingrate, center_frequency, frequency_width, 1,1);
	bs.reset ();
